...
function validateTTL(options) {
  const { ttl = 0 } = { ...options };
  validateInt32(ttl, 'options.ttl', 0);
  return ttl;
}
...
class ResolverBase {
  constructor(options = undefined) {
    ...
    const ttl = validateTTL(options);
    // If we are building snapshot, save the states of the resolver along
    // the way.
    if (isBuildingSnapshot()) {
      this[kSnapshotStates] = { timeout, tries, ttl };
    }
    this[kInitializeHandle](timeout, tries, ttl);
  }

  [kInitializeHandle](timeout, tries, ttl) {
    const { ChannelWrap } = lazyBinding();
    this._handle = new ChannelWrap(timeout, tries, ttl);
  }
  ...
  [kDeserializeResolver]() {
    const { timeout, tries, ttl, localAddress, servers } = this[kSnapshotStates];
    this[kInitializeHandle](timeout, tries, ttl);
    if (localAddress) {
